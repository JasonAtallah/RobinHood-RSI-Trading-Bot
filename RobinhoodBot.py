...
def run(sc):
    ...
    for key in historical_quotes["results"][0]["historicals"]:
        if currentIndex >= len(historical_quotes["results"][0]["historicals"]) - (
            rsiPeriod + 1
        ):
            if (
                currentIndex >= (rsiPeriod - 1)
                and datetime.strptime(key["begins_at"], "%Y-%m-%dT%H:%M:%SZ").minute
                == 0
            ):
                currentSupport = 0
                currentResistance = 0
    ...
    if len(closePrices) > (rsiPeriod):
        ...
        if (
            rsi[len(rsi) - 1] <= 30
            and float(key["close_price"]) <= currentSupport
            and not enteredTrade
        ):
        ...
        if (
            rsi[len(rsi) - 1] >= 70
            and float(key["close_price"]) >= currentResistance
            and currentResistance > 0
            and enteredTrade
        ):
